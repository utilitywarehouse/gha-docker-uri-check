...
function fetch(url, opts) {
	...
	return new fetch.Promise(function (resolve, reject) {
		...
		const send = (options.protocol === 'https:' ? require$$3 : http$1).request;
...
function createDockerClient(registry) {
    let token;
    async function fetchApi(path, config) {
        ...
        async function fetchToken(realm, service, scope) {
            const result = await fetch(realm +
                `?service=${encodeURIComponent(service)}&scope=${encodeURIComponent(scope)}`, {
                headers: {
                    Authorization: "Basic " +
...
function dockerImageURIFinder(registries) {
    const regexes = registries.map((registry) => 
    ...
    function parseYamlFile(path) {
        if (!yamlFiles.has(path)) {
            const contents = fs$1.readFileSync(path).toString();
    ...
    function uriFromKustomize(change) {
        ...
        for (const document of documents) {
            ...
            YAML.visit(document, {
                Scalar(_, node, path) {
                    ...
                    if (!registries.some((registry) => imageName.startsWith(registry.endpoint))) {
                        return YAML.visit.SKIP;
                    }
    ...
    const finders = [uriFromKustomize, uriFromRegex];
                ...
                for (const change of chunk.changes) {
                    ...
                    for (const finder of finders) {
                        ...
                        try {
                            ...
                            console.error(`Error trying to extract URI via ${finder.name}:`, error);
                            continue;
                        }
                        if (uri) {
                            uris.push(uri);
